...
С S-A3 приложение в образе запускается пользователем `node`. Код в `/app`
принадлежит root и только читается; писать можно только в `.next/cache`.
Команды обслуживания (`prisma migrate deploy`, `db:rename-questions`,
`prisma db seed`) работают от `node` — проверено. Если когда-нибудь разовой
задаче понадобится root, это `docker compose exec -u root app …`, осознанно
и один раз, а не поводом вернуть root образу.

Движок шахмат закреплён на версии (`stockfish=18-r0` в Dockerfile). Упала
сборка на этой строке — версия ушла из репозитория Alpine; новую ставить
осознанно, с прогоном `smoke:bot` и `smoke:magnus`.

## Сбои

Ошибки пишутся с меткой уровня (docs/ARCHITECTURE.md, «Уровни сбоев»):

```bash
docker compose -f docker-compose.prod.yml logs app --since 24h | grep '\[сбой:'
```

`[сбой:2]` — комната ушла на карантин: в строке ключ комнаты и игра, ниже
стек. `[сбой:3]` — сломалось что-то общее; процесс при этом работает, но
смотреть надо сразу. Оповещения хозяину пока нет — только лог.
...
**После выкладки безопасности** (ветка `security`, docs/SECURITY.md) —
руками, потому что смоуки по бою этого не видят:

```bash
# 1. Заголовки на месте: CSP, HSTS, nosniff.
curl -sI https://твой-домен/games | grep -iE 'content-security|strict-transport|nosniff'

# 2. Сокет пускает свой сайт: проверка Origin сверяет Origin с Host, а его
#    передаёт Caddy. Откройте комнату в браузере — состояние должно прийти.

# 3. Адрес клиента настоящий, а не адрес Caddy (S-R2): после пары входов
#    в журнале безопасности адреса разные.
docker compose -f docker-compose.prod.yml logs app --since 10m | grep '\[безопасность\]'

# 4. Сбоев нет.
docker compose -f docker-compose.prod.yml logs app --since 10m | grep '\[сбой:'
```

Ещё глазами: страница экрана в OBS (источник «Браузер») показывает игру, в
консоли браузера на сайте нет нарушений CSP. И ожидаемое: в первые полчаса
уборщик проставит отсчёт старым комнатам без людей (S-D2), и пустые комнаты,
висевшие с прошлых выкладок, исчезнут.
