...
Ставится пакетом в образ (`apk add stockfish`) и живёт **отдельным процессом**:
в наш бандл он не попадает, и его лицензия на наш код не распространяется. Путь
задан в образе переменной `CHESS_ENGINE_PATH=/usr/bin/stockfish`.
