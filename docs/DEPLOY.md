...
- лимиты («не больше N попыток входа», «не больше N писем») тоже живут в
  памяти: перезапуск их обнуляет, а второй инстанс вёл бы свои счётчики.
...
## Адрес клиента: почему верим `X-Forwarded-For`

Лимиты «с одного адреса» берут адрес игрока из заголовка
`X-Forwarded-For`, самый правый в цепочке (`src/server/client-address.ts`,
[SECURITY.md](./SECURITY.md) S-R2). Верить заголовку можно только потому,
как устроен стек:

- **Caddy 2 не доверяет входящему `X-Forwarded-For`** и перезаписывает его
  адресом, с которого пришёл запрос. В `Caddyfile` нет `trusted_proxies`, и
  добавлять его без нужды нельзя: тогда Caddy начнёт передавать то, что
  вписал клиент.
- **Приложение опубликовано только на `127.0.0.1:3000`** и в сети compose.
  Прийти мимо Caddy с поддельным заголовком запрос может только с самой
  машины.

Что ломает правило:

- **Порт приложения открыли наружу** — любой впишет себе любой адрес и
  обойдёт лимиты. Не открывать.
- **Перед Caddy встал ещё один прокси** (Cloudflare, балансировщик) — все
  запросы придут «с одного адреса», и лимит начнёт бить по всем сразу. Тогда
  нужен `trusted_proxies` на адреса этого прокси и пересмотр
  `client-address.ts`.

Проверка после выкладки: в логе Caddy и в `[безопасность]`-записях приложения
(S-H1) адреса игроков разные, а не один адрес контейнера Caddy.

## Журнал безопасности

Неудачные входы, сработавшие лимиты, отказы экрана и чужого сайта, смены
почты и пароля пишутся строкой с меткой `[безопасность]` и JSON в хвосте
(что именно — [SECURITY.md](./SECURITY.md), S-H1):

```bash
docker compose -f docker-compose.prod.yml logs app --since 24h | grep '\[безопасность\]'
```

Что искать: много «вход: неверный логин или пароль» на один логин — подбор
пароля; много «отказ по лимиту» с одного адреса — чей-то скрипт; все адреса
одинаковые — Caddy перестал передавать настоящий адрес (см. «Адрес клиента»
выше). Отказы по лимитам приглушены — не больше трёх строк в минуту на адрес,
так что три строки подряд значат «и дальше то же самое».

Журнал живёт столько, сколько логи Docker; своего хранилища у него нет.
...
**По приложению на домашнем экране** (версия 3.0) — четыре проверки, все
дешёвые:

```bash
# Манифест отдаётся своим типом, а не как попало.
curl -sI https://твой-домен/manifest.webmanifest | grep -i content-type

# Воркер отдаётся так, чтобы браузер перепроверял его каждый раз.
curl -sI https://твой-домен/sw.js | grep -i 'content-type\|cache-control'

# Страница «нет сети» открывается сама по себе.
curl -s -o /dev/null -w '%{http_code}\n' https://твой-домен/offline

# Тег, который Next не печатает сам, — без него айфон не откроет приложением.
curl -s https://твой-домен/games | grep -o 'apple-mobile-web-app-capable'
```

**Правя `public/sw.js`, поднимай в нём `VERSION`.** Файл в сборку не попадает
и хеша в имени не имеет; версия — единственное, чем старый кеш отличается от
нового. Забыть её не страшно (воркер всё равно обновится — он отдаётся с
`no-cache`, и при регистрации стоит `updateViaCache: "none"`), но кеш прошлой
версии останется лежать.

Проверить, что новый воркер встал, можно в инструментах разработчика:
Application → Service Workers, состояние `activated`. В Cache Storage при этом
обязана лежать одна-единственная страница «нет сети» — если там появилось
что-то с `/socket.io`, партии начнут разваливаться через раз.
