...
/**
 * Цвета письма — палитра платформы, заданная числами.
 *
 * Числами не по лени: почтовые клиенты не понимают переменных CSS, и токены
 * сюда не дотянуть. Значения повторяют `--color-*` из globals.css; меняя тему,
 * меняй и здесь.
 */
const ACCENT = "#a855f7";
const INK = "#f3eeff";
...
export function buildLetter(parts: LetterParts): Letter {
  ...
  const button = action
    ? `<p style="margin:22px 0 0">
         <a href="${escape(action.url)}" style="display:inline-block;padding:12px 22px;border-radius:12px;background:${ACCENT};color:#ffffff;font-weight:600;font-size:15px;text-decoration:none">${escape(action.label)}</a>
       </p>
...
<body style="margin:0;padding:24px;background:${SURFACE};font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif">
  <table role="presentation" style="max-width:520px;margin:0 auto;border-collapse:collapse">
    <tr><td style="padding:0 0 18px;font-size:20px;font-weight:700;color:${INK}">Плати<span style="color:${ACCENT}">тутка</span></td></tr>
