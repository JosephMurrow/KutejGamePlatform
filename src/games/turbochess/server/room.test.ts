...
describe("бот разговаривает", () => {
  /**
   * Зерно партии здесь постоянное. От зерна зависят ходы бота, а проверки
   * ниже ждут конкретных событий партии — «бот срубил», «бот потерял
   * фигуру». При случайном зерне примерно одна партия из двадцати шла мимо:
   * бот ставил мат раньше, чем терял хоть что-то дороже пешки, и проверка
   * падала раз в десяток прогонов всего набора. Здесь проверяется, что
   * комната зовёт бота говорить в нужный момент, а не то, как играет
   * помощник-человек, — поэтому партия одна и та же ход в ход.
   */
  const SEED = 1;
