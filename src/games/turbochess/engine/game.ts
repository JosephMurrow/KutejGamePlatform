...
export class TurboGame {
  ...
  /**
   * Событие «Загула» поменяло позицию мимо хода (docs/MODES.md, режим 12).
   *
   * Что именно оно делает с доской, знает режим: карту тянет комната, правку
   * считает `modes/binge.ts`, а фасаду остаётся то же, что и алко-штрафу, —
   * поставить новую позицию на место текущей и посмотреть, не кончилась ли
   * этим партия. Ходом событие не считается: ни очередь, ни счёт ходов от него
   * не двигаются, кроме тех событий, которые очередь и возвращают.
   */
  reshape(position: Position): Outcome | null {
    if (this.ended) return null;
