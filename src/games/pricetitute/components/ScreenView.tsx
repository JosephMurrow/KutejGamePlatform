 ...
 */
function useSettled(
  players: readonly GamePlayerPayload[],
): ReadonlySet<string> {
...
function screenName(
  player: GamePlayerPayload | undefined,
 ...
 */
function joinLink(state: GameStatePayload): string {
...
function JoinBoard({ state }: { state: GameStatePayload }) {
 ...
 */
function JoinCorner({ state }: { state: GameStatePayload }) {
  const link = joinLink(state);
  if (link === "") return null;
...
function title(state: GameStatePayload): string {
  switch (state.phase) {
    case "waiting":
      return "Перерыв";
    case "ready":
